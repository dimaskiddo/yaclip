...
- After each successful render, a `{i + 1}_{Title-Case}.txt` is written alongside the MP4 inside the `{VIDEO_ID_UCASE}` subdirectory.
...
- Output: `./workspace/clips/{VIDEO_ID_UCASE}/{i + 1}_{Title-Case}.mp4` at configured resolution.
...
| Final clips | `./workspace/clips/` | `{VIDEO_ID_UCASE}/{i}_{Title-Case-Dash}.mp4` |
| Clip metadata (title/caption/desc/hashtags) | `./workspace/clips/` | `{VIDEO_ID_UCASE}/{i}_{Title-Case-Dash}.txt` (auto-generated per clip in detected language) |
