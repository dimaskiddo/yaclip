...
flowchart TD
    ...
    subgraph S2["2. Ingestion Engine"]
        ...
        Download["yt-dlp Download<br/>(Bun JS + GitHub extractors)"] --> SaveVid["Save ./workspace/videos/[ID_UCASE].mp4"]
        Download --> SaveHeatmap["Save heatmap JSON<br/>(if YouTube provides it)"]
        SaveVid --> ExtractAudio["FFmpeg: Extract audio<br/>./workspace/audios/[ID_UCASE].aac"]
    end
...
The pre-ranking, candidate-margin, target-clip-count, and duration-enforcement steps are skipped entirely — clips render at exactly the user's boundaries (only a `start < end` safety check, no `MIN_CLIP_SECONDS` floor). Each line is `START - END` with an optional `| CONTENT_TYPE` suffix (e.g. `1:30 - 2:30 | JUST_CHAT`) that pins the layout for that range; a line without the suffix falls back to auto content-type detection for that window. A pinned type overrides the video-level detected type and is never overridden by the LLM. By default, each range is still sliced and STT-transcribed, then sent through the same batched LLM used by auto mode (`target_clips = len(ranges)`, so nothing is dropped) purely for **titling** — title/caption/description/hashtags are mapped back by index, boundaries never change. `--no-metadata` skips STT+LLM entirely; clips get a default `Manual_<start>_<end>` title and no `.txt` sidecar. Either way, results pass to the Review Gate like auto mode.
...
**Mode B — GAMING_SOLO_BOTTOM (mirrored variant):**
Identical crops and identical panel logic to GAMING_SOLO above — the only difference is the final `vstack` order is flipped (gameplay top, facecam bottom instead of facecam top, gameplay bottom). This type is **pin/override-only**: never auto-detected by the content-type detector and never assigned by the selection LLM, since top/bottom cam preference isn't visually detectable. Reachable via a manual timerange `\| GAMING_SOLO_BOTTOM` suffix or `video_processing.content_type_override: "GAMING_SOLO_BOTTOM"`. Subtitles still burn lower-center, so they land over the facecam panel on this variant.
...
- After each successful render, a `{NN}_{Title-Case}.txt` is written alongside the MP4 inside the `{VIDEO_ID_UCASE}` subdirectory. `NN` is the 1-based zero-padded index (`max(2, digits(total))` digits).
...
- Output: `./workspace/clips/{VIDEO_ID_UCASE}/{NN}_{Title-Case}.mp4` at configured resolution. `NN` = 1-based zero-padded index, width = `max(2, digits(total))`.
...
|---|---|---|
| Raw video | `./workspace/videos/` | `{VIDEO_ID_UCASE}.mp4` |
| Audio track | `./workspace/audios/` | `{VIDEO_ID_UCASE}.aac` |
...
| Final clips | `./workspace/clips/` | `{VIDEO_ID_UCASE}/{NN}_{Title-Case-Dash}.mp4` (zero-padded index) |
| Clip metadata (title/caption/desc/hashtags) | `./workspace/clips/` | `{VIDEO_ID_UCASE}/{NN}_{Title-Case-Dash}.txt` (zero-padded index, auto-generated per clip in detected language) |
