...
13. **Zero-padded clip filenames**: rendered clips and sidecar metadata use a zero-padded index (`01_`, `02_`, …) with width derived from total clip count (minimum 2 digits). Ensures consistent sort order in file managers and Clips Manager — `01`–`09` for ≤9 clips, `001`–`999` for 100+. Generated by `clip_stem()` in `src/core/utils.py`.