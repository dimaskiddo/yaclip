...
MAX_POPUP_SECONDS: float = 15.0  # Donation alerts are short; longer = persistent background.
POPUP_WINDOW_COVERAGE: float = 0.7  # Reject intervals covering >=70% of the window (scene change).
SCAN_PAD_SECONDS: float = 3.0  # Pad the scan window so a clip-spanning popup is still a minority.
...
MAX_POPUP_AREA_FRAC: float = 0.40  # Larger than this is a scene change, not an alert card.
...
BOTTOM_STRIP_FRAC: float = 0.88  # Boxes centred below this (full-width ticker) are dropped.
FULLWIDTH_FRAC: float = 0.70  # Boxes spanning >=70% width (ticker / lower-thirds) are dropped.
OVERLAY_DEFAULT_SAMPLE_STEP: float = 0.5  # ~2 fps sampling cadence for overlay scan.
OVERLAY_MAX_SCAN_SAMPLES: int = 200  # Upper bound on frames sampled to limit cost on long windows.
OVERLAY_MORPH_KERNEL_SIZE: int = 9  # Kernel size for morphological close (bridges small gaps).
OVERLAY_MAX_GAP_SECONDS: float = 1.5  # Max gap between detections still considered the same popup.
...
GAMEPLAY_PAN_DEADZONE_FRAC: float = 0.06  # Fraction of width — centre deadzone (no pan below this).
GAMEPLAY_PAN_MAX_VELOCITY_FRAC: float = 0.012  # Max pan speed as fraction of width per frame.
GAMEPLAY_PAN_MAX_DEVIATION_FRAC: float = 0.12  # Max dev from frame centre as fraction of width.
...
YOLO_CLUSTER_DIST_FRAC: float = 0.15  # YOLO box cluster distance threshold × frame diagonal.
...
FACECAM_DOMINANT_AREA_FRAC: float = 0.55  # Facecam covers >55% of frame → render as full-face
...
HUD_SCORE_THRESHOLD_GAMING_HINT: float = 0.0015  # Relaxed when YouTube category says "Gaming".
...
DONATION_HSV_ORANGE_LOWER: tuple[int, int, int] = (5, 100, 100)  # Orange/alert hue lower.
DONATION_HSV_ORANGE_UPPER: tuple[int, int, int] = (25, 255, 255)  # Orange/alert hue upper.
DONATION_MIN_AREA: int = 2000  # Min contour area (px) for donation-alert detection.
DONATION_MAX_AREA: int = 50000  # Max contour area (px); larger = scene change, not alert.
...
PODCAST_CROP_ASPECT: float = 9.0 / 16.0  # Full-height crop for single-speaker PODCAST (9:16).
COLLAB_CROP_HEIGHT_FRAC: float = 0.4  # Mode C bottom panel (collab face) is ~40% of frame height.
SOLO_CROP_HEIGHT_FRAC: float = 0.5  # Mode B panels (facecam/gameplay) are ~50% of frame height.
