...
- **Bulk Input:** Multiple ranges entered one per line in a textarea (WebUI) or via a `.txt` file upload or `--timerange-file` CLI flag (`src.core.utils.parse_timerange_line` / `load_timerange_file`). Each line is `START - END` with an optional `| CONTENT_TYPE` suffix (e.g. `1:30 - 2:30 | JUST_CHAT`) pinning that clip's layout; a line without the suffix falls back to auto content-type detection for that window. A pinned type (case-insensitive, matched against `ContentType`) overrides the video-level detected type and is never overridden by the LLM. Each line is parsed into a separate clip job.
...
ai_pipeline:
  stt:
    ...
    cloud:
      ...
      base_url: null                   # null = provider default; set for OpenRouter, ElevenLabs, or self-hosted OpenAI-compatible STT
      api_key: "your-api-key-here"
      model: "gemini-2.5-flash"        # Used for google; openai uses this model name against the endpoint
